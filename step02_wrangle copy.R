...
df <- df %>% 
  ...
  mutate(
    ...
    blocked_their_home = ifelse(event_type =="BLOCKED_SHOT" & event_team_abbr == away_abbreviation, 1, 0), # flip
    blocked_their_away = ifelse(event_type =="BLOCKED_SHOT" & event_team_abbr == home_abbreviation, 1, 0), # flip
  ...
  mutate(
    ...
    away_score_endof1st     = sum(event_type=="GOAL" & event_team_abbr==home_abbreviation & period<=1), 
    home_score_endof2nd     = sum(event_type=="GOAL" & event_team_abbr==home_abbreviation & period<=2), 
    away_score_endof2nd     = sum(event_type=="GOAL" & event_team_abbr==home_abbreviation & period<=2), 
    home_score_endof3rd     = sum(event_type=="GOAL" & event_team_abbr==home_abbreviation & period<=3), 
    away_score_endof3rd     = sum(event_type=="GOAL" & event_team_abbr==home_abbreviation & period<=3), 
    ...
    blocked_their_home_cum  = cumsum(blocked_their_home),
    blocked_their_away_cum  = cumsum(blocked_their_away),
...
df.game.results <- df %>%
  ...
  summarise(
    ...
    special_game            = last(special_game),
    ...
    game_seconds            = last(game_seconds),
    last_goal_seconds       = last(last_goal_seconds),
    last_goal_frac_reg      = last(last_goal_frac_reg),
    last_goal_frac_actual   = last(last_goal_frac_actual),
    score_before_last_goal  = last(score_before_last_goal),
    score_after_last_goal   = last(score_after_last_goal),
    score_diff_last_goal    = last(score_diff_last_goal),
    home_abbreviation       = first(home_abbreviation),
    away_abbreviation       = first(away_abbreviation),
    home_faceoffs           = last(faceoff_won_home_cum), 
    away_faceoffs           = last(faceoff_won_away_cum), 
    home_corsi              = last(corsi_event_home_cum), 
    away_corsi              = last(corsi_event_away_cum), 
    home_fenwick            = last(fenwick_event_home_cum), 
    away_fenwick            = last(fenwick_event_away_cum), 
    home_blocked            = last(blocked_home_cum), 
    away_blocked            = last(blocked_away_cum), 
    home_blocked_their      = last(blocked_their_home_cum),
    away_blocked_their      = last(blocked_their_away_cum),
    home_shots              = last(shot_home_cum), 
    away_shots              = last(shot_away_cum), 
    home_missed             = last(missed_home_cum), 
    away_missed             = last(missed_away_cum), 
    home_penalties          = last(penalty_home_cum), 
    away_penalties          = last(penalty_away_cum), 
    home_penalties_min      = last(penalty_min_home_cum),
    away_penalties_min      = last(penalty_min_home_cum),
    home_penalties_maj      = last(penalty_maj_home_cum),
    away_penalties_maj      = last(penalty_maj_home_cum),
    home_hits               = last(hits_home_cum), 
    away_hits               = last(hits_away_cum), 
    .groups = "drop"
  ) %>%
  mutate(
...
df.game.results.long <- bind_rows(
  ...
  df.game.results %>%
    transmute(
      ...
      blocked_their         = home_blocked_their,
      shots                 = home_shots,
      missed                = home_missed,
  ...
  df.game.results %>%
    transmute(
      ...
      blocked_their         = away_blocked_their,
      shots                 = away_shots,
      missed                = away_missed,
